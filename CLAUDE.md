...
Never add `Co-Authored-By`, `Generated by`, or any AI attribution/marketing lines to commit messages.
...
| "connect via cdp" | `desh connect --cdp` |
| "patch figma" | `desh patch` |
| "unpatch figma" | `desh unpatch` |
...
| "link components" | `desh components link` |
| "show linked" | `desh components linked` |
| "diff variants" | `desh components diff` |
| "push variants to Figma" | `desh components push` |
| "pull variants from Figma" | `desh components pull` |
...
Detects monorepo structure, finds CSS files with tokens, locates component directories, and auto-discovers the linked Figma library (via remote component instances + REST API).
...
Connects directly to Figma via Chrome DevTools Protocol. Much faster (~10x lower latency) but requires patching Figma's binary to re-enable remote debugging.

```bash
# One-time: patch Figma
desh patch

# Connect with CDP
desh connect --cdp

# Or set env var to use CDP for all commands
export DESH_USE_CDP=1
desh connect
```

**Trade-offs:**
- Requires Full Disk Access on macOS (System Settings → Privacy & Security)
- Figma updates may break the patch (run `desh patch` again)
- Violates Figma ToS (use at your own risk)

**Commands:**
- `desh patch` — Patch Figma to enable CDP
- `desh unpatch` — Restore Figma to original state
- `--cdp` flag or `DESH_USE_CDP=1` — Use CDP for any command
...
Link code components to Figma library component sets, then diff/push/pull variant changes:

```bash
desh components link               # Auto-match code↔Figma by name
desh components linked             # Show linked components + variant counts
desh components unlink "Button"    # Remove a link
desh components diff               # Compare variants: code vs Figma
desh components push               # Push missing code variants → Figma
desh components pull               # Pull missing Figma variants → code
```

Requires `FIGMA_API_TOKEN` and a linked library (auto-discovered by `desh init`).
